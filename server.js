...
const TRANSCRIPT_API_BASE = 'https://www.youtubetranscript.dev/api/v2';
...
async function providerTranscript(videoUrl, lang='') {
  if (!TRANSCRIPT_API_KEY) return null;
  const body = {
    video: videoUrl,
    source: 'auto',
    allow_asr: true,
    format: { timestamp: true, paragraphs: true, words: false }
  };
  if (lang) body.language = lang;
  const r = await fetch(`${TRANSCRIPT_API_BASE}/transcribe`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${TRANSCRIPT_API_KEY}`, 'Content-Type': 'application/json', 'Accept': 'application/json' },
    body: JSON.stringify(body)
  });
  let j = {};
  try { j = await r.json(); } catch {}
  if (!r.ok) {
    const e = new Error(j?.message || j?.error?.message || `Transcript provider returned ${r.status}`);
    e.status = r.status; e.provider = true; throw e;
  }
  if (j.status === 'completed' && j.data?.transcript) return j.data;
  if (j.job_id) {
    for (let i=0; i<36; i++) {
      await new Promise(resolve => setTimeout(resolve, 5000));
      const jr = await fetch(`${TRANSCRIPT_API_BASE}/jobs/${encodeURIComponent(j.job_id)}?include_segments=true`, {
        headers: { 'Authorization': `Bearer ${TRANSCRIPT_API_KEY}`, 'Accept': 'application/json' }
      });
      let jj = {};
      try { jj = await jr.json(); } catch {}
      if (jr.status === 200 && jj.status === 'completed' && jj.data?.transcript) return jj.data;
      if (jj.status === 'failed') throw new Error(jj.error || 'ASR transcription failed.');
    }
    throw new Error('Transcript is taking longer than expected. Please try again.');
  }
  throw new Error('Transcript provider returned an unexpected response.');
}

function normalizeProviderData(data, videoId, lang='') {
  const t = data?.transcript || {};
  const raw = Array.isArray(t.segments) ? t.segments : [];
  const segments = raw.map(x => {
    const startMs = Number(x.start ?? x.start_ms ?? 0);
    const start = startMs > 100000 ? startMs / 1000 : startMs;
    return { text: String(x.text || '').trim(), start, duration: Number(x.duration || 0), timestamp: formatTranscriptTime(start) };
  }).filter(x => x.text);
  return {
    videoId,
    ...
    languageName: languageName(t.language || lang || ''),
    source: t.source || data.source_kind || 'auto',
    segments,
    text: t.text || segments.map(x => x.text).join(' ')
  };
}
...
app.get('/api/transcript', async (req, res) => {
  ...
  // Preferred production path: caption extraction with ASR fallback.
  if (TRANSCRIPT_API_KEY) {
    try {
      const data = await providerTranscript(`https://www.youtube.com/watch?v=${videoId}`, lang);
      const normalized = normalizeProviderData(data, videoId, lang);
      if (!normalized.segments.length && !normalized.text) throw new Error('No transcript text was returned.');
      return res.json({ ...normalized, languages: normalized.language ? [{ code: normalized.language, name: languageName(normalized.language) }] : [] });
    } catch (err) {
      console.error('Transcript provider:', err.message);
      // Continue to the legacy extractor as a fallback for caption-only cases.
    }
  }
  ...
  try {
    ...
    if (!segments.length) return res.status(404).json({ error: TRANSCRIPT_API_KEY ? 'No transcript was returned. Please try again or select another language.' : 'Transcript service is not configured yet. Add TRANSCRIPT_API_KEY to enable ASR fallback for videos without accessible captions.' });
    const detectedLang = rows[0]?.lang || lang || '';
    const meta = await youtubeOembed(videoId);
    return res.json({ videoId, title: meta.title || 'YouTube Video', author: meta.author || '', language: detectedLang, languageName: languageName(detectedLang), source: 'captions', languages: detectedLang ? [{ code: detectedLang, name: languageName(detectedLang) }] : [], segments, text: segments.map(x=>x.text).join(' ') });
  } catch (err) {
    ...
    return res.status(502).json({ error: TRANSCRIPT_API_KEY ? 'Transcript could not be generated for this video. The video may be unavailable, restricted, or the transcript provider may be temporarily unavailable.' : 'Transcript provider is not configured. Add TRANSCRIPT_API_KEY in your hosting environment to enable transcripts for videos without accessible captions.' });
  }
});
